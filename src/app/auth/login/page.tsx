...
export default function LoginPage() {
  ...
  const { login, signInWithGoogle, user, isLoading, error: authError } = useAuth()
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'No se pudo iniciar sesion. Intenta de nuevo.')
    } finally {
    ...
    <main className="relative min-h-screen overflow-hidden bg-[#e6eadc] text-[#173c3a]">
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        ...
        <svg viewBox="0 0 800 800" className="absolute inset-0 h-full w-full opacity-70">
          ...
          <rect width="800" height="800" fill="url(#login-grid)" />
          <path
            d="M120 620C200 520 280 500 380 560C470 610 560 620 670 540C740 490 820 450 900 510"
            fill="none"
          ...
          />
          <path
            d="M70 520C150 420 240 420 330 470C420 520 520 520 620 470C700 430 760 390 830 440"
            fill="none"
            stroke="rgba(23,60,58,0.14)"
            strokeWidth="3"
            strokeLinecap="round"
          />
          <circle cx="620" cy="260" r="40" fill="rgba(23,60,58,0.08)" />
        </svg>
      </div>
          ...
          <div className="space-y-8 rounded-[2rem] border border-[#173c3a]/10 bg-white/95 p-10 shadow-[0_24px_80px_rgba(23,60,58,0.08)]">
            ...
            <div>
              <h1 className="text-5xl font-editorial font-semibold tracking-[-0.04em] sm:text-6xl">
                Tu puesto en la mesa del atlas.
              </h1>
              <p className="mt-6 max-w-xl text-base leading-8 text-[#47615a]">
                Inicia sesión con correo o Google y comparte el sabor que conecta a tu región con el
                mundo.
              </p>
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-[1.5rem] border border-[#173c3a]/10 bg-[#eff5ee] p-6">
                <p className="text-xs uppercase tracking-[0.24em] text-[#173c3a]/70">Rápido</p>
                <p className="mt-3 text-sm leading-7 text-[#47615a]">
                  Accede en segundos con Google y empieza a aportar.
                </p>
              </div>
              <div className="rounded-[1.5rem] border border-[#173c3a]/10 bg-[#eff5ee] p-6">
                <p className="text-xs uppercase tracking-[0.24em] text-[#173c3a]/70">Seguro</p>
                <p className="mt-3 text-sm leading-7 text-[#47615a]">
                  Protegemos tu cuenta para que tu historia quede siempre en el atlas.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="relative rounded-[2rem] border border-[#173c3a]/10 bg-white/95 p-10 shadow-[0_24px_80px_rgba(23,60,58,0.08)]">
            ...
            <div className="flex flex-col gap-6">
              <div>
                ...
                <h2 className="mt-3 text-3xl font-semibold text-[#173c3a]">Bienvenido de nuevo</h2>
              </div>
